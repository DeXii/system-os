...
export async function afterWorkoutComplete(plan: WorkoutPlan): Promise<void> {
  ...
  const sessionType =
    kind === 'hift' ? 'hift' : kind.startsWith('gpp_') ? 'strength' : kind === 'warmup' || kind === 'stretch' ? 'recovery' : 'strength';
  ...
  if (kind !== 'legacy') {
    const { incrementWorkoutTypeStat } = await import('./workout-stats');
    await incrementWorkoutTypeStat(kind, today);
  }

  const { recomputeFitnessLevels } = await import('./progression-engine');
  ...
  await emitKernel('foundation', `Тренировка завершена (${kind})`, 'success', TASK_KEYS.foundationWorkout);
  emitOsRefresh();
}
